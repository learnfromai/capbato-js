  ...
  validateLoginCommand(data: unknown): LoginUserCommand {
    ...
    if (!data || typeof data !== 'object') {
      throw new ValidationError(
        [{ code: 'invalid_type', path: ['data'], message: 'Invalid login data - expected object', expected: 'object', received: typeof data }],
        'Invalid login data'
      );
    }
    ...
    if (!request.password || typeof request.password !== 'string') {
      throw new ValidationError(
        [{ code: 'invalid_type', path: ['password'], message: 'Password is required', expected: 'string', received: typeof request.password }],
    ...
    if (request.email && typeof request.email === 'string') {
      ...
      throw new ValidationError(
        [{ code: 'invalid_type', path: ['email', 'username'], message: 'Email or username is required', expected: 'string', received: 'undefined' }],
