...
export class SqliteScheduleRepository implements IScheduleRepository {
  ...
  private initializeTable(): void {
    ...
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_schedules_doctor ON schedules(doctor_name);
  ...
  async create(schedule: Schedule): Promise<Schedule> {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO schedules (id, doctor_name, date, time, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
  ...
  async update(schedule: Schedule): Promise<Schedule> {
    ...
    const stmt = this.db.prepare(`
      UPDATE schedules 
      SET doctor_name = ?, date = ?, time = ?, updated_at = ?
      WHERE id = ?
    `);
  ...
  async findConflicts(schedule: Schedule): Promise<Schedule[]> {
    let sql = `
      ...
      AND date = ?
    `;
    const params = [schedule.doctorName, schedule.dateString];
